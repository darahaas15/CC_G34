...
std::string NodeIfElse::to_string()
{
    ...
    if (checkIfNum(condition->to_string()))
    {
        if (std::stoi(condition->to_string()) == 0)
        {
            return !else_cond ? "" : else_cond->to_string();
        }
        else
        {
            return !if_cond ? "" : if_cond->to_string();
        }
    }

    if (!else_cond && !if_cond)
    {
        return "(if-else " + condition->to_string() + " " + " " + ")";
    }
    if (!if_cond)
    {
        return "(if-else " + condition->to_string() + " " + " " + else_cond->to_string() + ")";
    }
    if (!else_cond)
    {
        return "(if-else " + condition->to_string() + " " + if_cond->to_string() + " " + ")";
    }
